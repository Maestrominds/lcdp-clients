<script>
    ...
    const filtered = $derived(() => {
        let items = inventory.filter((i) =>
            allowedCategories.includes(i.category?.toLowerCase()),
        );
        if (search)
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">
                Manage your stock levels and reorder items
            </p>
        </div>
    ...
    {:else}
        <div class="card">
            <table>
                <thead
                    ><tr
                        ><th>ITEM NAME</th><th>CATEGORY</th><th>QUANTITY</th><th
                            >MIN. STOCK</th
                        ><th>VENDOR</th><th>ACTIONS</th></tr
                    ></thead
                >
                <tbody>
                    {#each filtered() as item}
                        <tr>
                            ...
                            <td style="color:{qtyColor(item)};font-weight:600"
...
{#if showAddModal}
    <div class="modal-overlay" onclick={() => (showAddModal = false)}>
        <div class="modal-card" onclick={(e) => e.stopPropagation()}>
            ...
            <div class="form-row">
                <div>
                    <label class="field-label">Quantity</label><input
                        type="number"
                        step="any"
                ...
                <div>
                    <label class="field-label">Unit</label><select
                        bind:value={newItem.unit}
                        ><option>kg</option><option>L</option><option
                            >units</option
                        ></select
                    >
                </div>
            ...
            <div class="form-row">
                <div>
                    <label class="field-label">Min. Stock</label><input
                        type="number"
                        step="any"
                ...
                <div>
                    <label class="field-label">Vendor</label><input
                        type="text"
                        bind:value={newItem.vendor}
            ...
            <div class="modal-actions">
                ...
                <button class="btn btn-primary" onclick={saveItem}
                    >{editingId ? "Update" : "Save"} Item</button
                >
            </div>
        </div>
    </div>
