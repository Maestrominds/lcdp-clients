<script>
    ...
    onMount(async () => {
        try {
            payables = (await api.getPayables()) || [];
        } catch (e) {
        } finally {
            loading = false;
        }
    ...
    const toDueTime = (payable) => {
        const raw = payable.due_date || payable.dueDate;
        const time = raw ? Date.parse(raw) : NaN;
        return Number.isNaN(time) ? null : time;
    };
    const orderedPayables = $derived(
        [...payables].sort((a, b) => {
            const rankA = statusRank.get(a.status) ?? 3;
            const rankB = statusRank.get(b.status) ?? 3;
...
<div class="payables-page">
    ...
    <div class="summary-row">
        <div class="card summary-card">
            ...
            <p class="summary-value">₹{totalOutstanding.toFixed(2)}</p>
        </div>
        <div class="card summary-card">
            <p class="summary-label">Pending</p>
            <p class="summary-value" style="color:var(--warning-orange)">
                ₹{totalPending.toFixed(2)}
            </p>
        </div>
        <div class="card summary-card overdue-card">
            <p class="summary-label">Overdue</p>
            <p class="summary-value">₹{totalOverdue.toFixed(2)}</p>
        </div>
    </div>
    ...
    {:else}
        <div class="card">
            <table>
                <thead
                    ><tr
                        ><th>VENDOR</th><th>BILL REFERENCE</th><th>AMOUNT</th
                        ><th>DUE DATE</th><th>STATUS</th><th>ACTIONS</th></tr
                    ></thead
                >
                <tbody>
                    {#each orderedPayables as p}
                        <tr>
                            <td class="fw500">{p.vendor}</td>
                            <td class="muted">{p.txn_id || "—"}</td>
                            <td class="fw500">₹{p.amount.toFixed(2)}</td>
                            <td
                                style="color: {p.status === 'overdue'
                                    ? 'var(--error-red)'
                                    : 'inherit'}; font-weight: {p.status ===
                                'overdue'
                                    ? '600'
                                    : '400'}">{p.due_date || p.dueDate}</td
                            >
                    ...
                    {:else}
                        <tr
                            ><td colspan="6" class="empty-cell">No payables</td
                            ></tr
                        >
                    {/each}
                </tbody>
