<script>
    ...
    onMount(async () => {
        ...
        if (data && revenueCanvas) {
            ...
            console.warn("Canvas not found or data missing:", {
                data,
                revenueCanvas,
            });
        }
    ...
    {:else if data}
        <div class="kpi-row">
            ...
            <KpiCard
                ...
                trendUp={data.ordersChange > 0}
            />
            <KpiCard
                label="AVG ORDER VALUE"
                value="₹{data.avgOrderValue}"
                trend="{Math.abs(data.avgOrderValueChange)}%"
                trendUp={data.avgOrderValueChange > 0}
            />
        </div>
                    ...
                    {#each data.topItems || [] as item}
                        <div class="top-item-row">
                            ...
                            <div class="top-item-info">
                                ...
                                <p class="top-item-sub">
                                    {item.unitsSold} sold · ₹{item.revenue}
                                </p>
                            </div>
                        </div>
                    {/each}
